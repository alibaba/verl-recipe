...
async def _stream_and_record(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Session %s closed before streaming record (race), response already sent",
                trial_id,
            )
...
def start_proxy_server(
    ...
    actor = ProxyServerActor.options(
        name=PROXY_ACTOR_NAME,
        # The proxy is pinned to the head node, which typically runs with
        # num-cpus=0; don't request any CPU for the actor itself.
        num_cpus=0,
        scheduling_strategy=ray.util.scheduling_strategies.NodeAffinitySchedulingStrategy(
            node_id=head_node_id,
            soft=False,
        ),
    ).remote(load_balancer, model_path, host, port, tool_format, debug, session_dump_dir)
