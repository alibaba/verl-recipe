...
REMOTE_AGENT_POLL_INTERVAL          Seconds between async-run status polls (default: 5.0)
...
REMOTE_JOB_ID                       Job ID for grouping trials in the harbor frontend (default: verl-default)
...
class RemoteAgentConfig:
    ...
    # Interval between status polls when running a remote trial via the
    # async submit + poll API (see AgentRunClient.run_async_task).
    ...
    def from_env(cls) -> RemoteAgentConfig:
        ...
        if v := os.getenv("REMOTE_AGENT_POLL_INTERVAL"):
