...
class RemoteAgentLoop(AgentLoopBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.remote_job_id: str = remote_cfg.job_id
        ...
        self.poll_interval: float = remote_cfg.poll_interval
    ...
    async def _ensure_inference_worker(self) -> None:
        ...
        tool_format = self.config.actor_rollout_ref.rollout.multi_turn.format
    ...
    async def _submit_with_retry(
        ...
        for attempt in range(self.max_retries):
            if attempt > 0:
                # Reset session before retry to clear partial recordings
                ...
                try:
                    ...
                    logger.warning("Failed to reset session %s before retry (non-fatal): %s", trial_id, e)
    ...
    async def _run_remote_trial(
        ...
        Uses the async submit + poll API instead of a single blocking POST held
        open for the entire trial.  A long-lived request can have its underlying
        TCP connection silently dropped by the Kubernetes ``Service`` (conntrack
        idle timeout); the blocking POST would then hang until ``harbor_timeout``
        (potentially many hours), freezing the whole rollout gather and blocking
        training.  Polling submits quickly, then queries a short-lived status
        endpoint with retryable requests, bounding the total wait via
        ``poll_timeout``.
        """
        ...
        return await client.run_async_task(
            poll_interval=self.poll_interval,
            poll_timeout=self.harbor_timeout,
            task_path=task_path,
            job_id=job_id,
            task_id=task_id,
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        try:
            ...
            #    Fetch BEFORE delete — the session is removed in the finally block.
            ...
            if self._inference_worker is not None:
                self._inference_worker.clear_session_cache(trial_id)
            try:
                await self._delete_session(proxy_url, trial_id)
            except Exception as e:
                logger.warning("Failed to delete session %s (non-fatal): %s", trial_id, e)
    ...
    def _reconstruct_output(
        ...
        Token IDs and logprobs are read from *worker_cache* (populated by
        ``InferenceWorkerClient`` during inference).  If a cache entry is
        missing (e.g. worker restarted), the completion text is
        re-tokenized and logprobs default to 0.0.
