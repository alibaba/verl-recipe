...
> **与 `recipe/remote_agent` 的关系**：本 recipe 的核心路径（proxyserver + RemoteAgentLoop）已在 commit `264e6d7` 解耦重构为独立 recipe `recipe/remote_agent`（面向上游贡献）。两者是**平行关系，互不依赖**：
>
> - **`agentic`（本 recipe）**：生产全家桶——支持 standalone proxy 部署（设 `PROXY_SERVER_URL` 后 proxy 跑在外部 K8s）、disagg 训练（`agentic_disagg_main`）、per-step 计时（`timed_trainer`）、任务提交（`serversdk`），与 Harbor/ACK 环境深度集成；配置经 yaml → 环境变量注入 Ray worker。
> - **`remote_agent`**：框架无关的解耦版——agent 框架通过 runner 注册表插拔（Harbor 只是其中一个 runner），纯 yaml 配置，仅支持 Ray-actor proxy 模式。
>
> 选型：生产训练（standalone proxy、disagg、计时）用 `agentic`；上游贡献 / 接入新 agent 框架用 `remote_agent`。
...
├── train_agentic_hybrid_vllm_sweagent.sh      # 启动脚本：Agentic + Hybrid引擎 + vLLM + SWE-Agent
...
脚本 [`train_agentic_hybrid_vllm_sweagent.sh`](train_agentic_hybrid_vllm_sweagent.sh) 通过 `ENVIRONMENT` 环境变量选择运行环境，支持 **Docker**（本地）和 **ACK**（Kubernetes）两种模式。脚本会自动设置对应的 `environment_import_path` 和 `environment_kwargs`。
